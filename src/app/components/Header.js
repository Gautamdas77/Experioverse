...
export default function Header() {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`lg:hidden fixed inset-0 top-0 bg-ev-navy/95 backdrop-blur-lg transition-all duration-400 ${
          isMobileMenuOpen
        ...
        <div className="flex flex-col items-center justify-center h-full gap-1 px-6">
          {navLinks.map((link, i) => (
            <Link
              ...
              className={`text-white text-2xl font-semibold py-3 px-6 rounded-xl hover:bg-white/10 transition-all duration-300 ${
                isMobileMenuOpen
          ...
          <Link
            href="/contact"
            onClick={() => setIsMobileMenuOpen(false)}
            className={`mt-6 inline-flex items-center gap-2 px-8 py-3.5 bg-ev-accent hover:bg-ev-accent-hover text-white text-lg font-semibold rounded-full transition-all duration-300 ${
              isMobileMenuOpen
                ? 'opacity-100 translate-y-0'
                : 'opacity-0 translate-y-4'
            }`}
